...
const Home = ({navigation, props, route}) => {
  ...
  const getStoreList = (
    ...
    global.new_keyword =
      keyword === undefined ? '' : global.keyword ? global.keyword : keyword;
    ...
    var storeId =
      store_id == '' ? '' : global.pickStyleId ? global.pickStyleId : '';
    console.log(
      'Url is ====>>>>>',
      `${BASE_URL}/store/list2?price_to=${global.new_to_price}&price_from=${global.new_from_price}&keyword=${global.new_keyword}&service_id=${global.new_service}&miles=${global.new_miles}&latitude=${lat}&longitude=${long}&store_id=${storeId}`,
    );
    axios
      .get(
        `${BASE_URL}/store/list2?price_to=${global.new_to_price}&price_from=${global.new_from_price}&keyword=${global.new_keyword}&service_id=${global.new_service}&miles=${global.new_miles}&latitude=${lat}&longitude=${long}&store_id=${storeId}`,
  ...
  const mapView = () => {
    return (
      <MapView
        ...
        initialRegion={{
          latitude:
            storeList[0]?.latitude === undefined
              ? Number(global.latitude)
              : storeList[0]?.latitude,
          longitude:
            storeList[0]?.longitude === undefined
              ? Number(global.longitude)
    ...
    return (
      <View style={styles.searchMainView}>
        <Pressable
          style={[
            styles.searchView,
            {width: showFilter == false ? '78%' : '100%'},
          ]}
          onPress={() => navigation.navigate('HomeScreenSearch')}>
          <Text style={{padding: 15, color: 'grey'}}>
            {global.keyword ? global.keyword : 'Search By Salon, Location'}
          </Text>
        </Pressable>
        {showFilter == false ? (
          <Pressable
  ...
  const resetFilterButton = () => {
    return (
      <View>
        ...
        <Pressable style={styles.closefilter} onPress={() => {setShowFilter(!showFilter);}}>
          <Image source={require('../../../Images/cross.png')}></Image>
        </Pressable>
      </View>
    );
  };
    ...
    return (
      <View style={styles.pickStyleView}>
        <Image
          source={{
            uri:
              updatedName.fav.data.upload_front_photo != null
                ? updatedName.fav.data.upload_front_photo
                : updatedName.fav.data.upload_back_photo != null
                ? updatedName.fav.data.upload_back_photo
                : updatedName.fav.data.upload_top_photo != null
                ? updatedName.fav.data.upload_top_photo
                : updatedName.fav.data.upload_right_photo != null
                ? updatedName.fav.data.upload_right_photo
                : updatedName.fav.data.upload_left_photo != null
                ? updatedName.fav.data.upload_left_photo
                : updatedName.fav.data.upload_left_photo,
          }}
          style={styles.pickStyleImage}
        />
      ...
      <View>
        <View
          ...
          <Pressable
            ...
            onPress={() => setViewHideShow(!viewHideShow)}>
            {viewHideShow == true ? (
              <Image
          ...
          <ScrollView style={{marginTop: 15}}>
            {storeList.length === 0 && isLoading === false ? (
              ...
              storeList?.map((res, index) => {
                let price =
                  res.min_male_price > res.min_female_price
                    ? res.min_female_price
                    : res.min_male_price;
                      ...
                      <View style={styles.contentView}>
                        ...
                        <View style={styles.contentView}>
                          <Text style={styles.time}>
                            {moment
                              .utc(res.opentime, 'HH:mm:ss')
                              .local()
                              .format('hh:mm A')}
                            -
                            {moment
                              .utc(res.closetime, 'HH:mm:ss')
                              .local()
                              .format('hh:mm A')}
                          </Text>
                          {res.is_available == 1 ? (
